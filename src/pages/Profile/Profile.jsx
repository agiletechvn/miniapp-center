...
class AccessKeyItem extends Component {
	...
	renderEdit() {
		return (
			<EditAccessKey
				onCancel={this.props.onEditSelect}
	...
	renderContent() {
		const { createdTime, expires, name } = this.props;
		return (
			<Confirm
				onConfirm={this.props.onRemoveAccessKey}
				confirm={this.state.remove}
			...
			>
				<span>
					...
					<dl className="dl-horizontal">
						<dt>Created:</dt>
						<dd>{fmtDate(createdTime)}</dd>
						<dt>Expires:</dt>
						<dd>{fmtDate(expires)}</dd>
					</dl>
					<div className="btn-group btn-edit-group">
						<button className="btn btn-xs" onClick={this.handleEditClick}>
							<i className="fa fa-fw fa-edit" aria-label="Edit" />
						</button>
						<button
							className="btn btn-danger btn-xs"
							onClick={this.handleRemoveClick}
						>
							<i className="fa fa-fw fa-trash" aria-label="Remove" />
						</button>
					</div>
				</span>
			</Confirm>
	...
	render() {
		...
		return (
			<GroupItem>
				{this.props.editAccessKey ? this.renderEdit() : this.renderContent()}
			</GroupItem>
		);
	}
}
...
const accessKeyItemFactory = function(item, idx) {
	return (
		<AccessKeyItem
			...
			editAccessKey={
				this.editAccessKey && this.editAccessKey.idx == idx
					? this.editAccessKey
					: null
			}
			idx={idx}
...
export default class Profile extends Component {
	...
	componentWillReceiveProps(props) {
		if (
			props.isStale &&
			(!this.props.isStale || props.isStale > this.props.isStale)
		) {
			props.onAccessKeys();
		}
	}
	...
	render() {
		...
		return (
			<Page>
				...
				<Panel>
					<Heading label="Access Keys">
						{isAdding ? null : (
							<button
								className="btn btn btn-xs btn-primary pull-right"
								onClick={this.handleClickAddBtn}
							>
								Add <i className="fa fa-key" />
							</button>
						)}
					</Heading>
					<Body>
						...
						<GroupItem>
							...
							<Busy isBusy={accessKeys.isFetching}>
								<ul className="list-group">
									{items.map(accessKeyItemFactory, this.props)}
								</ul>
							</Busy>
						</GroupItem>
					</Body>
