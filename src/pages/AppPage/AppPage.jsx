...
const depLink = function(dep, idx) {
	return (
		<Link to={`/app/${this}/deployments/${dep}`} key={`dep-${idx}`}>
			{dep}
		</Link>
	);
};
...
const findOwner = collaborators =>
	Object.keys(collaborators).find(filterPerm, collaborators);
