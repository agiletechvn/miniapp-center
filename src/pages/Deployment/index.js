...
import { getCollaborators } from '../Collaborator/actions';
...
const mapStoreToProps = (state, props) => {
	...
	const { params } = props;
	const pkgs =
		(selectedDeploymentHistory && selectedDeploymentHistory.items) || [];
	return {
		...params,
		...
		isFetching:
			selectedDeployment.isFetching || selectedDeploymentHistory.isFetching,
		authorization,
		deployments,
		metrics,
		action: selectedDeployment.action,
		label: selectedDeployment.isRefresh
			? null
			: selectedDeployment.label ||
			  ((selectedDeployment.package && selectedDeployment.package.label) ||
					'') ||
			  (pkgs[pkgs.length - 1] && pkgs[pkgs.length - 1].label),
...
const mapDispatchToProps = (dispatch, pps) => {
	...
	return {
		onGetCollaborators(value) {
			return _dispatch(getCollaborators, this, value);
		},
		onGetDeployment(value) {
			return _dispatch(getDeployment, this, value);
		},
		onRenameDeployment(newName) {
			return _dispatch(renameDeployment, this, { newName }).then(() =>
				dispatch(navigate(makeUrl(this.name, newName)))
			);
		},
		onRemoveDeployment() {
			return _dispatch(removeDeployment, this).then(
				({ isError }) =>
					!isError &&
					dispatch(
						navigate(
							makeUrl(
								this.name,
								null,
								this.deployments.filter(dep => dep != this.deployment)
							)
						)
					)
			);
		...
		onPatchRelease(label, updateMetadata) {
			const current =
				this.selectedDeployment && this.selectedDeployment.package;
			...
			if (
				updateMetadata.rollout > 100 ||
				(current &&
					current.label === label &&
					current.rollout == updateMetadata.rollout)
			) {
