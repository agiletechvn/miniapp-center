...
export default function History({
	...
	}) => onSelect(deployments[parseInt(idx, 10)].label);
	return (
		<table className="table  table-hover">
			...
			<tbody>
				{deploymentList(deployments, metrics).map(deploymentListTr, {
					currentUserEmail,
					active,
...
const deploymentListTr = function({ packageObject, metrics }, idx) {
	...
	const releaseSource =
		packageObject.releaseMethod === 'Promote'
			? `Promoted "${packageObject.originalLabel}" from "${
					packageObject.originalDeployment
			  }"`
			: packageObject.releaseMethod === 'Rollback'
				? `Rolled back v${parseInt(packageObject.label.substring(1)) - 1} to  ${
	...
	return (
		<tr
			key={`row-${idx}`}
			className={`${packageObject.disabled ? 'disabled-package' : ''} ${
				packageObject.label == active ? 'active' : ''
			}`}
			data-idx={idx}
			onClick={onSelect}
		>
			{[
				packageObject.label,
				<span className="magenta release" title={'' + packageObject.uploadTime}>
				...
				<PackageMetrics packageObject={packageObject} metrics={metrics}>
					{packageObject.isDisabled ? (
						<div className="green">Disabled: Yes</div>
					) : null}
					{packageObject.rollout && packageObject.rollout !== 100 ? (
						<div className="green">
							Rollout: {packageObject.rollout.toLocaleString()}%
						</div>
					) : null}
				</PackageMetrics>
			].map(mapCell)}
		...
		<td>
			<div className="btn-group btn-group-xs">
				<button
					action="revert"
					data-idx="0"
					className="btn btn-default undefined "
					aria-label="Revert"
				>
					Revert
...
function PackageMetrics({ packageObject, metrics, children }) {
	if (!packageObject || !metrics) {
		return (
			<span>
				<div className="magenta">No installs recorded</div>
				{children}
			</span>
		);
	}
	...
	const percentString =
		activePercent === 100.0
			...
			: activePercent === 0.0
				? '0%'
				: activePercent.toPrecision(2) + '%';
	const numPending = metrics.downloaded - metrics.installed - metrics.failed;
	return (
		<div>
			<div className="active green">{`Active: ${percentString} (${metrics.active.toLocaleString()} of ${metrics.totalActive.toLocaleString()})`}</div>
			<div className="total green">{`Total: ${metrics.installed.toLocaleString()}`}</div>
			{numPending > 0 ? (
				<div className="pending green">{`(${numPending.toLocaleString()} pending)`}</div>
			) : null}
			{metrics.failed ? (
				<div className="rollbacks red">{`Rollbacks: ${metrics.failed.toLocaleString()}`}</div>
			) : null}
			{children}
		</div>
	);
...
const deploymentList = (deployments = [], metrics = {}) =>
	deployments.map(function(packageObject) {
