...
export default class Deployment extends Component {
	...
	componentWillReceiveProps({ name, deployment, label, isStale, isFetching }) {
		if (
			...
			(isStale && isStale != this.props.isStale)
		) {
			const value = { name, deployment };
			this.props.onClearError();
			this.props.onGetCollaborators(value);
	...
	handlePatchRelease = updateMetadata => {
		this.props
			.onPatchRelease(this.props.label, updateMetadata)
	...
	makeBtns() {
		if (
			this.props.selectedDeploymentHistory &&
			this.props.selectedDeploymentHistory.length > 1
		) {
			return buttons;
		}
		if (
			!(this.props.selectedDeployment && this.props.selectedDeployment.package)
		) {
			return [buttons[1]];
		}
	...
	renderPromote(pkg) {
		const deployments = this.props.deployments.filter(
			isNotThis,
		...
		return (
			<PromotePackage
				value={{
					deployments,
					destinationDeploymentName: deployments[0],
					...pkg
				}}
	...
	render() {
		...
		const isCurrent = !pkg
			...
			: selectedDeployment.package &&
			  selectedDeployment.package.label === pkg.label;
		return (
			<Busy isBusy={isFetching}>
				<div className="relative-inner">
					<Confirm
						confirm={action == 'remove' ? deployment : null}
					...
					>
						<span className="edit-delete-header">
							<h2>
								...
								<Inplace
									value={deployment}
									onChange={this.props.onRenameDeployment}
								/>
							</h2>

							<button
								onClick={this.handleRemoveClick}
								className="btn btn-xs btn-danger btn-right pull-right"
					...
					<Busy isBusy={metrics.isFetching}>
						...
						<History
							...
							deployments={selectedDeploymentHistory}
							onSelect={this.props.onSelectDeploymentLabel}
						/>
					</Busy>
					<Panel>
						<Confirm
							confirm={action == 'revert' ? label : false}
						...
						>
							<Heading
								...
								label={isCurrent ? 'Current' : this.props.label}
							>
								{this.renderActions(pkg)}
							</Heading>
						</Confirm>
						<Notify
							parent={`deployment-release-${name}-${deployment}`}
						...
						{action === 'release' ? (
							...
							<Confirm
								confirm={deployment}
								onConfirm={this.handleConfirmClearHistory}
							...
							this.renderPromote(pkg)
						) : (
							<Package
								{...selectedDeployment}
								depKey={selectedDeployment.key}
								package={pkg}
							/>
						)}
					</Panel>
